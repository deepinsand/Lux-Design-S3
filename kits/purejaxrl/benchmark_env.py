...
if __name__ == "__main__":
    ...
    def run_episode(rng_key, state, env_params):
        def take_step(carry, _):
            ...
            return (rng_key, state), (obs, state, reward, done, info)
        _, (obs, state, reward, done, info) = jax.lax.scan(take_step, (rng_key, state), length=max_episode_steps, unroll=1)
    ...
    def benchmark_reset_jax_lax_scan_jax_step(rng_key):
        ...
        obs, state, reward, done, info = run_episode(subkey, state, env_params)
        jax.block_until_ready(state)
    ...
    def run_episode_and_reset(rng_key, env_params):
        ...
        def take_step(carry, _):
            ...
            return (rng_key, state), (obs, state, reward, done, info)
        _, (obs, state, reward, done, info) = jax.lax.scan(take_step, (rng_key, state), length=max_episode_steps)
        return obs, state, reward, done, info
    # compile the scan
    ...
    def benchmark_jit_reset_lax_scan_jax_step(rng_key):
        rng_key, subkey = jax.random.split(rng_key)
        obs, state, reward, done, info = run_episode_and_reset(subkey, env_params)
        jax.block_until_ready(state)
